...
import { View, Text, StyleSheet, TextInput, TouchableOpacity, ScrollView, Alert, ActivityIndicator, SafeAreaView, StatusBar, Platform } from 'react-native';
...
export default function EditQuestionScreen() {
  ...
  const fetchQuestion = async () => {
    try {
      const data = await db.getFirstAsync<QuestionData & { explanation_a: string; explanation_b: string; explanation_c: string; explanation_d: string }>(
        'SELECT * FROM questions WHERE id = ?', [Number(id)]
      );
      if (data) {
        setQText(data.question_text);
        setOptA(data.option_a); setOptB(data.option_b); setOptC(data.option_c); setOptD(data.option_d);
        setExpA(data.explanation_a ?? ''); setExpB(data.explanation_b ?? '');
        setExpC(data.explanation_c ?? ''); setExpD(data.explanation_d ?? '');
        setCorrectAnswer(data.correct_answer);
        setExplanation(data.explanation ?? '');
        setLanguage(data.language);
        setSelectedLevel(data.level ?? 'Foundation');
        setSelectedCategory(data.category ?? 'CTFL');
      } else { 
        if (Platform.OS === 'web') alert('Soal tidak ditemukan'); else Alert.alert('Error', 'Soal tidak ditemukan'); 
        router.back(); 
      }
    } catch (e) { 
  ...
  const handleSave = async () => {
    if (!qText || !optA || !optB || !optC || !optD) {
      if (Platform.OS === 'web') alert('Harap isi pertanyaan dan semua pilihan jawaban!'); else Alert.alert('Error', 'Harap isi pertanyaan dan semua pilihan jawaban!');
      return;
    }
    try {
      await db.runAsync(
        `UPDATE questions SET question_text=?, option_a=?, option_b=?, option_c=?, option_d=?, correct_answer=?, explanation=?, explanation_a=?, explanation_b=?, explanation_c=?, explanation_d=?, language=?, category=?, level=? WHERE id=?`,
        [qText, optA, optB, optC, optD, correctAnswer, explanation, expA, expB, expC, expD, language, selectedCategory, selectedLevel, Number(id)]
      );
      if (Platform.OS === 'web') { alert('Soal berhasil diubah'); router.back(); } else { Alert.alert('Sukses', 'Soal berhasil diubah', [{ text: 'OK', onPress: () => router.back() }]); }
    } catch (e) { 
      if (Platform.OS === 'web') alert('Gagal menyimpan soal'); else Alert.alert('Error', 'Gagal menyimpan soal'); 
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView style={styles.form} showsVerticalScrollIndicator={false}>
        ...
        <TextInput style={[styles.input, styles.textArea]} multiline value={qText} onChangeText={setQText} placeholder="Tulis pertanyaan..." placeholderTextColor="#94A3B8" />
        ...
        {[{ key: 'A', val: optA, set: setOptA, exp: expA, setExp: setExpA },
          ...
          <View key={key}>
            ...
            <TextInput style={styles.input} value={val} onChangeText={set} placeholder={`Pilihan ${key}`} placeholderTextColor="#94A3B8" />
            <Text style={styles.labelSub}>Penjelasan Pilihan {key}</Text>
            <TextInput style={[styles.input, styles.inputSub]} value={exp} onChangeText={setExp} placeholder={`Penjelasan pilihan ${key}...`} placeholderTextColor="#CBD5E1" multiline />
          </View>
        ))}
        ...
        <View style={styles.row}>
          {['A', 'B', 'C', 'D'].map((opt) => (
            <TouchableOpacity key={opt} style={[styles.btn, correctAnswer === opt && { backgroundColor: '#22C55E', borderColor: '#22C55E' }]} onPress={() => setCorrectAnswer(opt)}>
              <Text style={[styles.btnText, correctAnswer === opt && styles.activeText]}>{opt}</Text>
            </TouchableOpacity>
          ))}
        </View>

        <Text style={styles.label}>Pembahasan Umum</Text>
        <TextInput style={[styles.input, styles.textArea]} multiline value={explanation} onChangeText={setExplanation} placeholder="Penjelasan umum jawaban..." placeholderTextColor="#94A3B8" />
